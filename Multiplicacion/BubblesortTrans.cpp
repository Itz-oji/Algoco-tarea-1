...
// Función para multiplicar dos matrices optimizando la localidad de los datos
vector<vector<int>> multiplicarMatrices(const vector<vector<int>>& A, const vector<vector<int>>& B_T) {
    size_t filasA = A.size();
    size_t columnasA = A[0].size();
    size_t columnasB = B_T.size();  // Debido a que B está transpuesta

    // Crear una matriz resultado con tamaño adecuado inicializada a 0
    vector<vector<int>> resultado(filasA, vector<int>(columnasB, 0));

    // Multiplicación de matrices con B transpuesta
...
int main() {
    for(int i=0; i<10; i++){
        ...
        cout << "Transponiendo matriz B..." << endl;
        vector<vector<int>> matrizB_T = transponerMatriz(matrizB);

        // Multiplicar las matrices
        start = high_resolution_clock::now();
        cout << "Multiplicando matrices..." << endl;
        vector<vector<int>> resultado = multiplicarMatrices(matrizA, matrizB_T);
        stop = high_resolution_clock::now();
        duration = duration_cast<milliseconds>(stop - start);
